...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    let(:number_guessing) { double('random number double', value: 8) }
    ...
    context 'when min is 5 and max is 9' do
      subject(:game7) { described_class.new(5, 9, number_guessing) }

      it 'returns 7' do
        result = game7.make_guess
        expect(result).to be 7
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game8) { described_class.new(8, 9, number_guessing) }

      it 'returns 8' do
        result = game8.make_guess
        expect(result).to be 8
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game1) { described_class.new(0, 3, number_guessing) }

      it 'returns 1' do
        result = game1.make_guess
        expect(result).to be 1
      end
    end

    context 'when min and max both equal 3' do
      subject(:game3) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        result = game3.make_guess
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are not equal' do
      let(:random_number) { double('random number', value: 9) }
      subject(:game_not_over) { described_class.new(0, 9, random_number, 1) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:edge) { described_class.new(5, 8, number_range, 7) }
      ...
      it 'updates min to the same value as max' do
        expect(edge.min).to eq(8)
        expect(edge.max).to eq(8)
      end
