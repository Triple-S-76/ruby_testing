...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:game) { described_class.new(0, 9, number_guessing, 5) }

      it 'is game over' do
        expect(game.be_game_over).to eq true
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:game) { described_class.new(0, 9, number_guessing, 2) }

      it 'is not game over' do
        expect(game.be_game_over).to_not eq true
      end
    end
  end
