...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    let(:random_number) { double('Random Number', value: 99) }

    context 'when min is 5 and max is 9' do
      subject(:game) { described_class.new(5, 9, random_number) }
    ...
    context 'when min is 8 and max is 9' do
      subject(:game2) { described_class.new(8, 9, random_number) }

      it 'returns 8' do
        expect(game2.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game3) { described_class.new(0, 3, random_number) }

      it 'returns 1' do
        expect(game3.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game4) { described_class.new(3, 3, random_number) }
      
      it 'returns 3' do
        expect(game4.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:game5) { described_class.new(0, 9, random_number, 5) }
      let(:random_number) { double('random number', value: 5) }

      it 'is game over' do
        expect(game5.game_over?).to be true
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      subject(:game6) { described_class.new(0, 9, random_number, 7) }
      let(:random_number) { double('random number', value: 5) }

      it 'is not game over' do
        expect(game6.game_over?).to be false
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      subject(:edge_cases) { described_class.new(5, 8, number_range, 7)}

      before(:each) do
        edge_cases.update_range
      end

      it 'updates min to the same value as max' do
        expect(edge_cases.min).to eq(edge_cases.max)
      end

      it 'does not update max' do
        expect(edge_cases.max).to eq(8)
      end
    end
  end
end
