...
describe MagicSeven do
  ...
  describe '#subtract_four' do
    context 'when the previous step is 104' do
      it 'returns 100' do
        previous_step = 104
  ...
  describe '#divide_by_two' do
    context 'when the previous step is 100' do
      it 'returns 50' do
        previous_step = 100
  ...
  describe '#subtract_random_number' do
    subject(:my_game) { described_class.new(66) }
  ...
  describe '#play' do
    context 'when the random number is 99' do
      subject(:my_game1) { described_class.new(99) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = my_game1.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is -999' do
      subject(:my_game2) { described_class.new(-999)}
      # remove the 'x' before running this test
      it 'will return 7' do
        result = my_game2.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 987654321' do
      subject(:my_game3) { described_class.new(987654321) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = my_game3.play
        expect(result).to eq(7)
      end
    end
  end
