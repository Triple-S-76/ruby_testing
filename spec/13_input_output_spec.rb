...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_not_ended) { described_class.new(1, '9') }
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(game_not_ended).not_to be_game_over
      end
    end
  end
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        rejected_input = game_check.verify_input(user_input)
        expect(rejected_input).to be_nil
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_loop).to receive(:player_input).and_return('non valid 1', 'non valid 2', '3')
      end

      it 'completes loop and displays error message twice' do
        expect(game_loop).to receive(:puts).with('Input error!').twice
        # expect(game_loop).to receive('3').once
        game_loop.player_turn
      end
    end
  end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:my_game) { described_class.new(5, '5', 5) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
