...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:not_game_over) { described_class.new(1, '2') }
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        input2 = 'b'
        input3 = '3'
        allow(game_loop).to receive(:player_input).and_return(input1, input2, input3)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:bad_game) { described_class.new(5, '5', 76) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "That was hard. It took you 76 guesses!\n"
        expect { bad_game.final_message }.to output(congrats_phrase).to_stdout
      end
    end
  end
end
